...
impl Parsable<Content> for Course
where
    Course: GetHtmlExt,
{
    fn parse(&self, client: &mut AuthenticatedClient) -> anyhow::Result<Content> {
        ...
        for content_fr in document.select(&content_selector) {
            ...
            let mut builder = ContentBuilder::new(
                full_title
                    ...
                    .inner_html(),
                full_title.text().nth(1).expect("Failed to get type").into(),
            );
